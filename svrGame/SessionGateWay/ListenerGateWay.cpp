...
void ListenerGateWay::bindNodeId(SessionGateWay* pSession, int32 nodeId)
{
	auto t = m_mapSessionIdByNodeId.find(nodeId);
	if (t != m_mapSessionIdByNodeId.end())
	{
	...
	pSession->setGatewayId(nodeId);
	pSession->setCheck(true);
	m_mapSessionIdByNodeId[nodeId] = pSession->GetSessionId();
}
