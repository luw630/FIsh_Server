...
{
	log_dbg("rcv cmd(%d) len(%d)", packet->m_Command, packet->m_Length);
	App::GetInstance().getSeesionDb()->addBuffer(packet, GetSessionId(), packet->m_PacketParam);
	return 0;
}
...
void SessionProxy::OnConnected()
{
	...
	stConnServerInfo.m_entityType = emSvrType_Db;
