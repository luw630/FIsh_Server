...
int SessionProxy::OnPacket(PacketHeader* packet, Stream& stream)
{
	...
	else if (packet->m_Command == EInnerPacketCommand_Pong)
	{
		...
		if (proxyHead.m_srcType == emSvrType_Game)
		{
			Dispatcher_Games& dispatcher = App::GetInstance().getDispatcherPackets()->m_DispatcherGames;
			auto handler = dispatcher.GetPacketHandler(packet->m_Command);
			if (handler != nullptr) 
			{
				(dispatcher.*handler)(packet, Stream(stream.GetOffsetPointer(), stream.GetSize() - stream.GetOffset()), msgParamLen, msgParam);
			}
		}
	}
	return 0;
...
void SessionProxy::OnConnected()
{
	...
	stConnServerInfo.m_entityType = emSvrType_Account;
